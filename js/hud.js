...
// 패널 치수 (기준 960px 기준 — scale 적용으로 자동 확대)
const PANEL_W = 168;
const PANEL_H = 345;
const PAD     = 11;
...
export function renderHUD(p, a1, a2, a1v, a2v, L1, L2, m1 = 12, m2 = 6, massMode = 'STANDARD') {
  ...
  // 화면 폭에 따라 HUD 크기 선형 스케일 (960px → ×1.0, 1920px → ×1.8, 최대 ×2.0)
  ...
  p.text('\u03c91', cx, cy - R1 - 6);
  p.fill(C2[0], C2[1], C2[2], 100);
  p.text('\u03c92', cx, cy + R1 + 16);

  // ── 데이터 구분선 ──────────────────────────────────────────────────────────
  const dataY = DC_Y + R1 + 22;
  ...
  p.stroke(C1[0], C1[1], C1[2], 11);
  p.strokeWeight(2.5);
  p.rect(-1, -1, PANEL_W + 2, PANEL_H + 2, 6);
